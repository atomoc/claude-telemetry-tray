...
DEAD_RESTART_GAP = 120   # сек: мёртвый трей поднимаем быстро, но не штормим


def main():
    running = bool(tray_pids())
    wedged = looks_wedged()
    if running and not wedged:
        return   # трей жив и здоров

    # Два случая: трей мёртв (процесса нет — logout/краш/сон убили, автозапуск
    # срабатывает только на login) или завис (процесс есть, но индекс пуст).
    # Раньше сторож умел только второе; мёртвый трей висел, пока пользователь не
    # заметит. Мёртвый поднимаем быстро (120 с), зависший перезапускаем реже
    # (15 мин) — детект сессий не используем, он под планировщиком врёт.
    gap = DEAD_RESTART_GAP if not running else MIN_RESTART_GAP
    age = _last_restart_age()
    if age < gap:
        wlog("нужно поднять, но недавно (%.0f с назад) уже стартовал — жду" % age)
        return
    wlog("трей %s — (пере)запускаю"
         % ("не запущен" if not running else "завис (индекс пуст >5 мин)"))
    _mark_restart()
    restart_tray()
